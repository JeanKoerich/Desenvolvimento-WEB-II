    ...
    <div class="py-8">
        <div class="max-w-6xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="flex justify-between items-center mb-6">
                <form action="{{ url('livros/busca') }}" method="GET" class="flex items-center w-full max-w-md space-x-2">
                    <input type="text" name="busca" value="{{ $busca ?? '' }}" placeholder="Buscar título ou autor..."
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg shadow-sm focus:ring focus:ring-indigo-200">
                    <button type="submit" class="px-4 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700">
                        Buscar
                    </button>
                    @if (!empty($busca))
                        <a href="{{ url('livros') }}" class="px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700">X</a>
                    @endif
                </form>
                    ...
                    <tbody>
                        @forelse ($livros as $livro)
                            <tr class="border-t hover:bg-gray-50">
                                ...
                                <td class="py-3 px-6">{{ $livro->cliente->nome ?? 'Não vinculado' }}</td>
                                <td class="py-3 px-6 space-x-2">
                                    <a href="{{ route('livros.edit', $livro) }}" class="text-blue-600 hover:underline">Editar</a>
                        ...
                        @empty
                            <tr>
                                <td colspan="4" class="py-3 px-6 text-center text-gray-500">Nenhum livro encontrado.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
